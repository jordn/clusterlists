    ...
    $("li.user").droppable({
      ...
      drop: function(event, ui){
        ...
        if ($memberships.find("." + $draggable.data('list')).length == 0) {
          ...
          $memberships.append($("<li class='temp " + list_slug + "'></li>").append("<img class='spinner' src='rotate-large.gif'> " + $draggable.text() ));
          Meteor.call("AddUserToList", user_slug, list_slug, list_id, function(err,result){
              $memberships.find($('.temp.'+list_slug).remove());
          });  
        }
      }
    });
...
Template.friend.events({
  'click a.remove': function (event) {
    ...
    parent_list.remove();
    console.log('Removing ' + user_slug + ' from ' + list_slug);
